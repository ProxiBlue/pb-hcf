...
- **`simplify-pass` — new enrollable agent, `post-implementation` order 25 (before `codegraph-reviewer`@30).** Closes a real gap found via usage telemetry: the general-purpose `/simplify` and `/code-review` skills are registered fleet-wide but showed ZERO real invocations across every session transcript ever scanned — nobody was reaching for them, and it turned out the pipeline's existing reviewers (`codegraph-reviewer`, `graphiti-reviewer`, `security-quorum`, `mutation-tester`) all explicitly disclaim the reuse/simplification/efficiency axis ("NOT a style reviewer"), and `standards-enforcer` is scoped to pure style, forbidden from touching behavior. `simplify-pass` reviews the plan's whole diff against a YAGNI→reuse→stdlib→abstraction-load-bearing ladder, returns PASS or advisory PUSHBACK (file:line + a concrete suggested cut) — never blocks the commit, never edits code itself. Wire's enrollable-agent table grows to **17 agents**; `--enable-all` now includes it. `pipeline-audit`'s inline-only-reviewer list updated to include it (no plan-dir artefact, evidenced via commit-trailer/verdict-log fallback like the other inline reviewers).

- **`post-plan-playwright-bucket-split` + `pre-batch-playwright-floor-guard` — 2 new enrollable agents, first use of the `pre-batch` hook point (7 → 8 hooks in active use).** Land the JS-unit-first / Playwright-coverage-floor discipline from `pb-hcf-playwright-tdd` v0.4.1's `testing.md` into the actual task-file authoring and spawn-time enforcement, without touching HCF source (`hcf:plan-create`/`hcf:tdd-worker` stay untouched — confirmed against this repo's own "HCF source untouched" principle before designing these).
  - `post-plan-playwright-bucket-split` (`post-plan`, order 15 — between `devils-advocate`@10 and `pre-mortem`@20, and well before `post-plan-manual-test-plan`@50 so it mines the already-split sections). Splits each Rule-2 task's flat `## Requirements (Test Descriptions)` into `## Requirements — JS unit` + `## Requirements — Playwright` buckets by classifying each existing requirement (real browser/DOM/network/iframe/morph/session needed, or not), backfills `**Domain**:` frontmatter (checkout|payment|order|other), self-verifies via `playwright-floor-guard.sh`, and explicitly refuses to fabricate weak Playwright items just to pass the floor — under-floor tasks it can't confidently complete are surfaced under "Items for your consideration" for human authoring instead. Idempotent (skips files already split).
  - `pre-batch-playwright-floor-guard` (`pre-batch`, order 10 — first agent ever enrolled at this hook point; fires before every batch spawn including the first, so no separate `pre-implementation` companion was needed). Thin, deterministic, haiku-tier re-check of `playwright-floor-guard.sh` before each spawn — catches a task file thinned below the floor after `post-plan-playwright-bucket-split` ran (manual edit, or a prior batch's tdd-worker self-editing its own task file). Reports `STATUS: BLOCK` naming the specific task(s) to exclude from that batch's spawn; does not fix anything itself.
  - New hook script this pair depends on: `~/claude-skills-central/hooks/playwright-floor-guard.sh` (deterministic exit-code guard, same pattern as push-guard.sh/git-tree-guard.sh/test-gate.sh — landed 2026-08-06, fixed 2 integration bugs found during design: a heading-level mismatch that would have permanently blocked every floor-domain task, and a bold-markdown frontmatter-tag mismatch that would have silently let untagged floor-domain tasks bypass the floor entirely. Both caught and fixed before landing, tested against fixtures matching the real template).
  - Wire's enrollable-agent table grows to **16 agents at 8 hook points**; `--enable-all` now includes both. Not enabled by default anywhere (matches "ships dormant" convention) — opt in per project via `/pb-hcf:wire --enable=post-plan-playwright-bucket-split,pre-batch-playwright-floor-guard`.
  - Origin: host chatroom thread `480b71f6` (2026-08-06/07) — pvcpipesupplies proposal to shift Rule-2 default from Playwright-always to JS-unit-first, approved with a required deterministic coverage-floor amendment after review flagged that the original prose-only "why not JS unit" guardrail would have let checkout/payment coverage thin silently. Mid-review also surfaced and fixed an unrelated pb-chatroom MCP identity-forging bug (see `pb-chatroom` v0.4.1 changelog) on the same thread.
