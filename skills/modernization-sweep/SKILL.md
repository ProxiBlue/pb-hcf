...
5. **Hand off — never self-merge.** This skill's job ends at the commit. Hand the branch to the standard review chain (the project's normal PR / review-agent flow, e.g. `codegraph-reviewer`, `security-quorum`, human review) exactly as any other feature branch would be. This skill MUST NOT merge, push directly to a protected branch, or fast-forward the branch itself under any circumstance — merging is always a separate, human- or reviewer-gated step.
